...
class CarWashGUI extends JFrame {
    ...
    private JTextArea activityLog;   // shows the log of activities (car arrivals, service, etc.)
    ...
    public CarWashGUI(int numPumps, int queueSize) {
        ...
        activityLog.setFont(new Font("Consolas", Font.PLAIN, 15));
        activityLog.setBackground(new Color(240, 240, 240));
        add(new JScrollPane(activityLog), BorderLayout.SOUTH);
...
class Pump extends Thread {
    ...
    public void run() {
        ...
        while (!ServiceStation.isSimulationEnded()) {
            ...
            ServiceStation.getFull().P();  // P on full: block if no cars
